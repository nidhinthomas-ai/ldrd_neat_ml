...
def process_image(image_path: str, output_dir: str, sam_model: SAMModel, mask_settings: Dict[str, Any], debug: bool = False) -> PandasDataFrame:
    """
...
def bubbleSAM (df: PandasDataFrame, output_dir: str, debug: bool = False) -> PandasDataFrame:
    """
    ...
    num_blobs = np.empty(shape=(df_new.shape[0]),
                         dtype=np.int64)
    sam_model = SAMModel(model_config='sam2_hiera_l.yaml', 
    ...
    for index, row in tqdm(df_new.iterrows(),
        ...
        masks_summary_DF = process_image(img_filepath, 
                                         output_dir, 
                                         sam_model, 
                                         mask_settings, 
                                         debug = debug)
        num_blobs_img = masks_summary_DF.shape[0]
        median_droplet_radii[index] = np.sqrt(np.median(masks_summary_DF["area"])/np.pi)

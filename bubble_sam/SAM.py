...
class SAMModel:
    ...
    def generate_masks(self, output_dir: str, image: np.ndarray, mask_settings: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        ...
        sam2 = build_sam2(self.model_config, self.checkpoint, device=self.device, apply_postprocessing=False)
    ...
    def mask_summary(self, masks: List[Dict[str, Any]]) -> PandasDataFrame:
        """
        ...
        """
        keys_to_remove = ["crop_box", "segmentation", "bbox", "predicted_iou", "stability_score", "point_coords"]
        mask_dict = [{k: v for k, v in d.items() if k not in keys_to_remove} for d in masks]
